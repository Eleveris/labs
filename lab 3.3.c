...
int inputFunc(FILE *inputFile, inputList *value);

int addNewName(namesList *name, inputList *input, int *pointer);

int addNemPayment(namesList *name, paymentList *payment, inputList *value);
...
int insertPayment(namesList *inPoint, paymentList *keyElement, inputList *values);

int relocateDate(inputList *values, namesList *startPoint);

int bubbleSort(int *values, int valuesCount);

dBlock *decimalDistribution(int *mediumValues, int valuesCount);

int printHist (dBlock *decimalBlocks, int numberOfElements);
...
int main() {
    ...
    fgets(scan,sizeof(scan)*MAXSTRSIZE,inputFile); // for skipping first str
    free(scan);
    ...
    while (1) //it is not a function and i don't know why it so
    {
        mediumValues=realloc(mediumValues,sizeof(int)*i+1);
...
int inputFunc(FILE *inputFile, inputList *value)
{
    ...
    if (NULL == fgets(scan,sizeof(scan),inputFile)) return 0;
    pos=strchr(scan,';');
    col=pos-scan;
...
int addNewName(namesList *name, inputList *value, int *pointer)
{
...
int addNemPayment(namesList *name,paymentList *payment,inputList *values)
{
...
//return link at equal or last element
paymentList *paymentsCheck(paymentList * const startPoint, inputList *date, int *pointer)
{
